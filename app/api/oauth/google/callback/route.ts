...
function popupResponse(status: "ok" | "error", message?: string) {
  const body = `<!doctype html><html><body><script>
    (function() {
      try {
        if (window.opener) {
          window.opener.postMessage({ source: "lifeos-oauth", provider: "google", status: ${JSON.stringify(status)}, message: ${JSON.stringify(message ?? "")} }, window.location.origin);
        }
      } catch (e) {}
      window.close();
...
export async function GET(req: NextRequest) {
  ...
  const cookieState = req.cookies.get(`${OAUTH_STATE_COOKIE_PREFIX}google`)?.value;
  ...
  res.cookies.set(`${OAUTH_STATE_COOKIE_PREFIX}google`, "", {
    path: "/",
    maxAge: 0,
  });
  return res;
